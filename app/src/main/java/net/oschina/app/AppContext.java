...
public class AppContext extends BaseApplication {
    ...
    private void init() {
        ...
        //2）初始化网络请求
        AsyncHttpClient client = new AsyncHttpClient(); /** android-async-http-1.4.9 **/
        PersistentCookieStore myCookieStore = new PersistentCookieStore(this); /*使用PersistentCookieStore永久化保存Cookie*/
    ...
    private void initLogin() {
        ...
        if (null != user && user.getId() > 0) { /* 如果用户存在，且id 大于0*/
            loginUid = user.getId(); /*那么，就获取用户id，并赋值给登录id使用*/
        } else {
            this.cleanLoginInfo(); /*如果用户不存在，则清除客户输入的错误的登录信息*/
        }
    }
